...
////////////////////////////////////////////////////////////
bool CursorImpl::loadFromPixels(const Uint8* pixels, Vector2u size,
                                Vector2u hotspot)
{
    release();
...
////////////////////////////////////////////////////////////
bool CursorImpl::loadFromPixelsARGB(const Uint8* pixels, Vector2u size,
                                    Vector2u hotspot)
{
    // Create cursor image, convert from RGBA to ARGB.
    XcursorImage* cursorImage =
        XcursorImageCreate(static_cast<int>(size.x), static_cast<int>(size.y));
    ...
    for (std::size_t pixelIndex = 0; pixelIndex < numPixels; ++pixelIndex)
    {
        cursorImage->pixels[pixelIndex] = static_cast<Uint32>(
            pixels[pixelIndex * 4 + 2] + (pixels[pixelIndex * 4 + 1] << 8) +
...
////////////////////////////////////////////////////////////
bool CursorImpl::loadFromPixelsMonochrome(const Uint8* pixels, Vector2u size,
                                          Vector2u hotspot)
{
    // Convert the image into a bitmap (monochrome!).
    // The bit data is stored packed into bytes. If the number of pixels on each
    // row of the image does not fit exactly into (width/8) bytes, one extra
    // byte is allocated at the end of each row to store the extra pixels.
    std::size_t packedWidth = (size.x + 7) / 8;
    std::size_t bytes = packedWidth * size.y;
    std::vector<Uint8> mask(
        bytes, 0); // Defines which pixel is opaque (1) or transparent (0).
    std::vector<Uint8> data(
        bytes, 0); // Defines which pixel is white (1) or black (0).
    ...
    for (std::size_t j = 0; j < size.y; ++j)
    {
        for (std::size_t i = 0; i < size.x; ++i)
        {
            ...
            // Choose between black/background & white/foreground color for each
            // pixel, based on the pixel color intensity: on average, if a
            // channel is "active" at 50%, the bit is white.
            int intensity =
                (pixels[pixelIndex * 4 + 0] + pixels[pixelIndex * 4 + 1] +
    ...
    Pixmap maskPixmap = XCreateBitmapFromData(
        m_display, XDefaultRootWindow(m_display),
    ...
    m_cursor = XCreatePixmapCursor(m_display, dataPixmap, maskPixmap, &fg, &bg,
                                   hotspot.x, hotspot.y);
...
bool CursorImpl::loadFromSystem(Cursor::Type type)
{
    ...
    case Cursor::SizeVertical:
        ...
        break;
    case Cursor::SizeLeft:
        shape = XC_left_side;
        break;
    case Cursor::SizeRight:
        shape = XC_right_side;
        break;
    case Cursor::SizeTop:
        shape = XC_top_side;
        break;
    case Cursor::SizeBottom:
